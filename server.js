...
const companyId=req=>{const actor=resolveActorContext(req);if(actor?.scope==='company')return actor.company_id||null;if(actor?.actor_type==='legacy')return isAdmin(req)?(req.body?.company_id||actor.company_id||null):(actor.company_id||null);return null};
const requireCreationCompany=(req,res,next)=>{const actor=resolveActorContext(req),cid=companyId(req);if(actor?.actor_type==='unresolved'||actor?.scope!=='company')return res.status(403).json({error:'Se requiere contexto empresarial'});if(!cid)return res.status(400).json({error:'company_id es obligatorio para crear este recurso'});req.resourceCompanyId=cid;next()};
