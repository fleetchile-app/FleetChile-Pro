...
function registerAdminRoutes(app,pool){
  ...
  const adminOnly=(req,res,next)=>{const legacy=req.user?.role_code==='admin'&&!req.user?.membership_id&&!req.user?.platform_membership_id;const allowed=(req.user?.scope==='platform'&&req.user?.permissions?.some(p=>p==='platform.companies.manage'||p==='platform.users.manage'))||legacy;if(allowed)return next();res.status(403).json({error:'Acceso restringido a administradores de plataforma'});};
