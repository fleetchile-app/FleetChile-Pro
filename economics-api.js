...
function registerEconomicsRoutes(app,pool){
  ...
  app.get('/api/economics/kpis',requirePermission('economics.read'),async(req,res)=>{
    ...
    try{
      ...
      const defined=trips.filter(t=>t.revenue_defined===true),missing=trips.filter(t=>t.revenue_defined!==true),economic=trips.map(t=>{const has=Number(t.recognized_costs)>0||t.approved_costs!==null||t.reconciled_costs!==null;const delta= t.revenue_defined===true&&has?Number(t.revenue_clp)-Number(t.recognized_costs):null;const status=!t.revenue_defined||!has?'missing_data':Number(t.approved_costs||0)>0?'provisional':'reconciled';const d=Number(t.snapshot_distance_km)>0?Number(t.snapshot_distance_km):Number(t.distance_km)>0?Number(t.distance_km):Number(t.route_distance_km)>0?Number(t.route_distance_km):null;return {delta,status,distance:d}});
      ...
      const [docs,maint,alerts,checks,fleet,fuel]=await Promise.all([
        ...
        q("select count(*) filter(where status='Pendiente') pending,count(*) filter(where status='En proceso') in_process,count(*) filter(where status='Completada') completed,count(*) filter(where status='Cancelada') cancelled,count(*) filter(where (next_due_date<current_date or (next_due_odometer_km is not null and t.km>=next_due_odometer_km)) and status not in ('Completada','Cancelada')) overdue from maintenance m left join trucks t on t.id=m.truck_id and t.company_id=m.company_id where m.company_id=$1"),
        ...
        q("select count(*) filter(where status='Aprobado') approved,count(*) filter(where status='Pendiente') pending,count(*) filter(where status in ('Rechazado','Reprobado')) rejected from vehicle_checklists vc join trips t on t.id=vc.trip_id where t.company_id=$1"),
