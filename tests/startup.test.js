...
test('runner incluye la fundación económica después de alertas operacionales',()=>{
  ...
  assert.equal(MIGRATION_FILES.at(-6),'022_platform_company_identity.sql');
  assert.equal(MIGRATION_FILES.at(-5),'023_platform_governance.sql');
  assert.equal(MIGRATION_FILES.at(-4),'024_platform_ownership.sql');
  assert.equal(MIGRATION_FILES.at(-3),'025_bootstrap_platform_owner.sql');
  assert.equal(MIGRATION_FILES.at(-2),'026_global_geography_catalog.sql');
  assert.equal(MIGRATION_FILES.at(-1),'027_seed_chile_dpa_2023.sql');
