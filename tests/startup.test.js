...
test('runner incluye la fundación económica después de alertas operacionales',()=>{
  ...
  assert.equal(MIGRATION_FILES.at(-5),'022_platform_company_identity.sql');
  assert.equal(MIGRATION_FILES.at(-4),'023_platform_governance.sql');
  assert.equal(MIGRATION_FILES.at(-3),'024_platform_ownership.sql');
  assert.equal(MIGRATION_FILES.at(-2),'025_bootstrap_platform_owner.sql');
  assert.equal(MIGRATION_FILES.at(-1),'026_global_geography_catalog.sql');
