...
test('runner incluye la fundación económica después de alertas operacionales',()=>{
  ...
  assert.equal(MIGRATION_FILES.at(-4),'022_platform_company_identity.sql');
  assert.equal(MIGRATION_FILES.at(-3),'023_platform_governance.sql');
  assert.equal(MIGRATION_FILES.at(-2),'024_platform_ownership.sql');
  assert.equal(MIGRATION_FILES.at(-1),'025_bootstrap_platform_owner.sql');
