...
function browserContext(){
 ...
 const context={console,document,Auth:{user:{id:1,role_code:'admin',company_id:null,permissions:[]}},alert:message=>alerts.push(message),setInterval(){},setTimeout(){},clearTimeout(){},URLSearchParams,FormData:class{constructor(target){return target.data}},fetch:async(url,opt={})=>{requests.push({url,opt});let data=[];if(url==='/api/companies')data=[{id:10,legal_name:'Empresa A'}];else if(url==='/api/dashboard')data={trucks:0,enroute:0,loads:0,alerts:0,fuel:0,km:0};else if(url==='/api/core/summary')data={};else if(url==='/api/drivers'&&opt.method==='POST')data={id:21,...JSON.parse(opt.body)};return {ok:true,status:200,async json(){return data}}}};
